...
func (i *Issuer) Issue(userID, tenantID int64, roles []int64, perms []string, refreshJTI string) (TokenPair, error) {
	now := time.Now().UTC()
...
func (i *Issuer) signed(userID, tenantID int64, roles []int64, perms []string, jti string, now time.Time, ttl time.Duration) (string, time.Time, error) {
	exp := now.Add(ttl)
	claims := Claims{
		TenantID: tenantID,
		Roles:    roles,
