...
func (i *Issuer) Issue(userID, tenantID uint64, roles []int64, perms []string, refreshJTI string) (TokenPair, error) {
	now := time.Now().UTC()
	access, accessExp, err := i.signed(userID, tenantID, roles, perms, "", now, i.accessTTL)
	if err != nil {
		return TokenPair{}, err
	}
	refresh, refreshExp, err := i.signed(userID, tenantID, nil, nil, refreshJTI, now, i.refreshTTL)
	if err != nil {
		return TokenPair{}, err
	}
	return TokenPair{
...
func (i *Issuer) signed(userID, tenantID uint64, roles []int64, perms []string, jti string, now time.Time, ttl time.Duration) (string, time.Time, error) {
	exp := now.Add(ttl)
	claims := Claims{
		TenantID: tenantID,
		Roles:    roles,
